...
void atk96_weather_damage(void) 
{
    ...
    if (weather_abilities_effect() && !(get_item_effect(bank_attacker, 1) == ITEM_EFFECT_SAFETYGOOGLES) &&
        !(ability_effect && (ability == ABILITY_MAGIC_GUARD || ability == ABILITY_OVERCOAT)) && is_bank_present(bank_attacker)) 
		{

        if (SANDSTORM_WEATHER) 
		{
            if (!(is_of_type(bank_attacker, TYPE_GROUND) || is_of_type(bank_attacker, TYPE_STEEL) || is_of_type(bank_attacker, TYPE_ROCK)) &&
			!(ability_effect && (ability == ABILITY_SAND_FORCE || ability == ABILITY_SAND_RUSH || ability == ABILITY_SAND_VEIL))) 
				{
		...
		{
            bool chilly_reception_immune = false; // Booleano local

            if (battle_weather.flags.chilly_reception_hail) 
			{
                chilly_reception_immune = true; // Imune se Hail foi definido por Chilly Reception
            }
            ...
            if (!(is_of_type(bank_attacker, TYPE_ICE)) && !(ability_effect && (ability == ABILITY_ICE_BODY || ability == ABILITY_SNOW_CLOAK))) 
			{
                if (!(status3[bank_attacker].underground || status3[bank_attacker].underwater)) 
				{
                    if (chilly_reception_immune) 
					{
                        damage = 0; // Sem dano se imune
                    } else 
					{
                        damage = ATLEAST_ONE(battle_participants[bank_attacker].max_hp >> 4);
                    }
...
bool entry_hazards_hook(void)
{
	...
	else if (side_affecting_halfword[banks_side].spikes_on && !(side_affecting_halfword[banks_side].spikes_damage_done) && GROUNDED(active_bank) &&
			not_magicguard(active_bank) && !has_hp_immunity(active_bank) && !is_on_boots)
	{
		...
		effect = 1;
			...
			!has_hp_immunity(active_bank) && !is_on_boots)	
	{
...
bool move_effect2_setter(void)
{
	...
	switch (BIC(*move_effect, EFFECT2_AFFECTSUSER))
    {
		...
		case 12: //clear stat changes
            ...
            break;
         case 13: //steal item
            if(!substitute && battle_participants[bank_attacker].current_hp && !battle_flags.flag_x4000000 && !WILD_ATTACKER)
            {
		...
		case 14: //smack poke down
			if (current_hp && new_battlestruct->bank_affecting[bank].smacked_down == 0 && (!GROUNDED(bank) || status3[bank].on_air ||
		...
		case 58: // Eerie Spell
			...
			break;
		case 59: // Chilly Reception
            if (MOVE_WORKED && !new_battlestruct->various.sheerforce_bonus)
            {
                bs_push_current(BS_CHILLY_RECEPTION);
            }
            break;
		case 60: // Chilly Reception
            if (MOVE_WORKED && !new_battlestruct->various.sheerforce_bonus)
            {
                bs_push_current(BS_CHILLY_RECEPTION);
            }
...
#define case_max 41
	...
	new_battlestruct->bank_affecting[bank_attacker].lastmove_fail = !MOVE_WORKED;
        ...
        switch (battle_scripting.cmd49_state_tracker)
        {
			...
			case 1://spiky shield recoil
						...
						attacker_struct->def_buff != 0)
				{
					...
					effect = 1;
						...
						!cant_poison(bank_target, bank_attacker, 0))
				{
			...
			case 11: //choice move update
			{
				if (!SEPARATE_COURSE)
				{
					...
					if ((CHOICE_ITEM(attacker_item) || battle_participants[bank_attacker].ability_id== ABILITY_GORILLA_TACTICS)
						&& last_used_move != MOVE_STRUGGLE && (hitmarker & HITMARKER_OBEYS) && (*choice_move == 0 || *choice_move == 0xFFFF))
					{
			...
			case 14: //refresh/restore defender's sprite
				if (!special_statuses[bank_target].restored_bank_sprite && (bank_target < gBattlersCount) &&
			...
			case 19: //setup buffer for conversion, sketch etc.
				if (!SEPARATE_COURSE)
				{
					...
					if ((hitmarker & HITMARKER_ATTACKSTRING_PRINTED) && last_used_move != 0 && last_used_move != MOVE_SKETCH)
			...
			case 24: //dual target moves
				if (!(hitmarker & HITMARKER_IMMOBILE_DUE_TO_STATUS) && (get_move_table_target(current_move,bank_attacker) == move_target_both &&
				battle_flags.double_battle && !protect_structs[bank_attacker].flag0_onlystruggle && !(hitmarker & HITMARKER_NO_ATTACKSTRING)
			...
			case 25: //triple target moves
						...
						!protect_structs[bank_attacker].flag0_onlystruggle)
				{
					if ((move_table[current_move].script_id == 23 || move_table[current_move].script_id == 192) &&
							move_outcome.explosion_stop) //stop explosion if damp on
					{
					...
					while (next_target < gBattlersCount &&
			...
			case 35: //dancer & instruct
			{
				...
				{
					while (new_battlestruct->various.secondary_dancer < gBattlersCount && !effect)
					{
			...
			case 38: //get burnt by Beak Blast
			{
			...
			case 40: //Clanging Scales & Clangorous Soulblaze
			{
				if ((current_move == MOVE_CLANGING_SCALES) && clanging_scales_stat())
					effect = 1;
				INC_END_EVENTS
				break;
			}
			case 41: //neutralizing_gas_leaving
	...
	if (current_move_turn < 2 && (battle_scripting.cmd49_state_tracker >= case_max)) 
	{
		...
		for (u8 i = current_move_turn+1; i < gBattlersCount; i++) 
		{
			for (u8 j = i+1; j < gBattlersCount; j++)
			{
				faster = 2;
...
bool healblock_forbidden_moves(u16 move, u8 with_leechseed)
{
    static const u16 healthblock_forbidden_moves[] = {MOVE_ABSORB,MOVE_MEGA_DRAIN,MOVE_GIGA_DRAIN,MOVE_LEECH_LIFE
			...
			,MOVE_PARABOLIC_CHARGE,MOVE_OBLIVION_WING,MOVE_SHORE_UP,MOVE_SLACK_OFF
			,MOVE_SYNTHESIS,MOVE_SWALLOW,MOVE_STRENGTH_SAP,MOVE_MILK_DRINK,MOVE_HEAL_ORDER
			,MOVE_INGRAIN,MOVE_LUNAR_DANCE,MOVE_MORNING_SUN,MOVE_MOONLIGHT,MOVE_AQUA_RING
			,MOVE_HEALING_WISH,MOVE_HEAL_PULSE,MOVE_SOFTBOILED,MOVE_REST,MOVE_WISH,MOVE_ROOST
			,MOVE_RECOVER,MOVE_FLORAL_HEALING,0xFFFF};
...
const u8 BadgeLvTable[] = {13,23,28,37,45,51,57,62,70,100};
u8 is_poke_disobedient(void) //0x08045CBC
{
    s32 rnd;
    s32 calc;
    u8 obedienceLevel = 0;

    if (battle_flags.double_battle || battle_flags.flag_x2000000)// @ Caso de batalha dupla ou condições especiais de batalha, sempre obedece.
        return 0;
    if (get_bank_side(bank_attacker) == 1) // @ Pokémon do lado oponente sempre obedece.
        return 0;

    if (HasObedientBitSet(bank_attacker)) // @ Determina o nível de obediência baseado nas insígnias do jogador.
    {	// @ Exceções em certos cenários de jogo.
    ...
    if (battle_participants[bank_attacker].level <= obedienceLevel)// @ Se o nível do Pokémon for menor ou igual ao de obediência, sempre obedece.
        return 0;
    rnd = (rng() & 255);
    ...
    // @ Pokémon pode desobedecer; verifica condições de movimentos especiais.
    ...
    {
        battlescripts_curr_instruction = (void*) 0x82db695; // Ignora enquanto dormindo.
        return 1;
    }

    rnd = (rng() & 255);// @ Randomiza obediência.
    calc = (battle_participants[bank_attacker].level + obedienceLevel) * rnd >> 8;
    if (calc < obedienceLevel)
    {
        calc = check_move_limitations(bank_attacker, bits_table[current_move_position], 0xFF);
        if (calc == 0xF) // @ Verifica se todos os movimentos estão indisponíveis.
        {
            battle_communication_struct.multistring_chooser = rng() & 3;
            battlescripts_curr_instruction = (void*) 0x82db6ad;// @ Todos os movimentos bloqueados: Loafing Around.
            return 1;
        }
        else // @ Escolhe movimento aleatório.
        {
            do
            {
            ...
            battlescripts_curr_instruction = (void*) 0x82db6a5;// Usa movimento aleatório.
    ...
    else
    {
        ...
        calc = (rng() & 255);// @ Chances de cair no sono.
        ...
        {
            // @ Verifica se há Pokémon em Uproar.
            int i;
            ...
            if (i == gBattlersCount)
            {
                battlescripts_curr_instruction = (void*) 0x82db6d9;// @ Dorme.
                return 1;
            }
        }
        calc -= obedienceLevel;// @ Última verificação: Autolesão.
        if (calc < obedienceLevel)
        {
			damage_calc(MOVE_POUND, move_table[MOVE_POUND].type, bank_attacker, bank_attacker, 64, 1);
            bank_target = bank_attacker;
            battlescripts_curr_instruction = (void*) 0x82db6f0;// Bate em si mesmo.
            hitmarker |= HITMARKER_IMMOBILE_DUE_TO_STATUS;
            return 2;
        }
        else
        {
            battle_communication_struct.multistring_chooser = rng() & 3;
            battlescripts_curr_instruction = (void*) 0x82db6ad; // @ Ignora comando (Loafing Around).
            return 1;
        }
    }
}
...
void atk77_set_protect_stuff(void)
{
    ...
    if (current_move_turn == gBattlersCount - 1)
        fail = 1;
    else
    {
...
void atk0C_datahpupdate(void)
{
	...
	else
	{
		...
		if (affected_by_substitute(bank) && damage_loc > 0)
		{
...
#define MOVE_MAX 621
	static const u16 metronome_forbidden_moves[] = {MOVE_AFTER_YOU, MOVE_ASSIST, MOVE_BELCH, MOVE_BESTOW,
			...
			MOVE_WIDE_GUARD, MOVE_OBSTRUCT, MOVE_CHLOROBLAST, MOVE_DOUBLE_IRON_BASH, MOVE_BODY_PRESS, MOVE_SNAP_TRAP,
			MOVE_STRANGE_STEAM, MOVE_FREEZING_GLARE, MOVE_THUNDEROUS_KICK, MOVE_FIERY_WRATH, MOVE_EERIE_SPELL,
			MOVE_CLANGOROUS_SOUL, MOVE_DRAGON_ENERGY, MOVE_FIERY_WRATH, MOVE_MIND_BLOWN, MOVE_PLASMA_FISTS,
			MOVE_PHOTON_GEYSER, 0xFFFF};
	do
	{
...
void atk13_printfromtable(void)
{
    ...
    bool adrenaline_orb_activated = false;
        ...
        || (new_battlestruct->side_affecting[get_bank_side(bank)].sticky_web_done && new_battlestruct->bank_affecting[bank].stat_lowered))
    {
        ...
        {
            if (get_bank_side(bank_target) != get_bank_side(bank_attacker) || new_battlestruct->side_affecting[get_bank_side(bank)].sticky_web_done)
            {
        ...
        if (adrenaline_orb_activated)
        {
            bs_push_current(BS_ADRENALINE_ORB);
        }
...
// @ Função de reversão de forma
void revert_form_change(bool mega_revert, u8 teamID, u8 side, struct pokemon* poke) 
{
    if (mega_revert) 
	{
        // @ Se mega_revert é verdadeiro, reverte Megaevoluções
        revert_mega_to_normalform(teamID, side);
        return;
    }

    u16 current_hp = get_attributes(poke, ATTR_CURRENT_HP, 0); // @ Obtém o HP atual
    u16 species = get_attributes(poke, ATTR_SPECIES, 0); // @ Obtém a espécie atual
    if (species == POKE_ZYGARDE_100 && battle_outcome) 
	{        // @ Caso especial para Zygarde Complete (100%)
        u16 base_species = POKE_ZYGARDE_10; // @ Verifica se deve reverter para Zygarde 50% com base nas flags
        ...
        set_attributes(poke, ATTR_SPECIES, &base_species); // @ Atualiza a espécie e recalcula os stats
        calculate_stats_pokekmon(poke);
        u16 total_hp = get_attributes(poke, ATTR_TOTAL_HP, 0); // @ Ajusta o HP atual se necessário
    ...
    if (species == POKE_MAGEARNA_ORIGINAL && battle_outcome) 
	{
        // @ Caso especial para Magearna Origin
        u16 base_species = POKE_MAGEARNA; // @ Define a espécie base
        if ((side && (new_battlestruct->party_bit.is_base_mag_ai & bits_table[teamID])) ||
            (!side && (new_battlestruct->party_bit.is_base_mag_user & bits_table[teamID]))) 
			{
            if (side) 
			{
                new_battlestruct->party_bit.is_base_mag_ai = 0; // @ Reseta o bit para AI
            } 
			else 
			{
                new_battlestruct->party_bit.is_base_mag_user = 0; // @ Reseta o bit para o jogador
            }
            base_species = POKE_MAGEARNA;
        }

        set_attributes(poke, ATTR_SPECIES, &base_species); // @ Atualiza a espécie
        calculate_stats_pokekmon(poke); // @ Recalcula os stats

        u16 total_hp = get_attributes(poke, ATTR_TOTAL_HP, 0); // @ Obtém o HP total
        if (current_hp > total_hp)
            set_attributes(poke, ATTR_CURRENT_HP, &total_hp); // @ Ajusta o HP atual se necessário
        else
            set_attributes(poke, ATTR_CURRENT_HP, &current_hp);
...
void atkE1_intimidate_loop(void)
{
	...
	battle_text_buff1[3] = 0xFF;
	while (bank_target < gBattlersCount)
	{
...
void atk02_attackstring(void)
{
    if (!battle_execution_buffer)
    {
		...
		if ((battle_participants[bank_attacker].ability_id == ABILITY_PROTEAN) && current_move != MOVE_STRUGGLE && !move_outcome.failed &&
...
#define exp_for_poke damage_loc
	...
	{
		return;
	} // @ Aguarda a execução de ações anteriores
    ...
    // @ Tabela de Level Caps baseada em badges
    const u8 level_caps[] = {13, 23, 28, 37, 45, 51, 57, 62, 70, 100};
    const u16 badge_flags[] = {0x867, 0x868, 0x869, 0x86A, 0x86B, 0x86C, 0x86D, 0x86E}; // Flags das badges

    // @ Função para obter o Level Cap atual baseado nas badges
    u8 get_current_level_cap(void)
    {
        for (u8 i = 0; i < sizeof(badge_flags) / sizeof(badge_flags[0]); i++)
        {
            if (!GET_CUSTOMFLAG(badge_flags[i])) // @ Se a flag da badge não está ativa
                return level_caps[i];
        }
        return level_caps[sizeof(level_caps) / sizeof(level_caps[0]) - 1]; // @ Retorna o último valor (100)
    }

    // @ Nível máximo permitido no momento
    u8 current_level_cap = get_current_level_cap();
    switch (*tracker)
    {
		case 0: // @ Verifica se o Pokémon deve ganhar experiência
			...
			{
				*tracker = 6; // @ Define o último estado
				break;
			}
			battle_stuff_ptr->got_exp_from |= bits_table[battle_team_id_by_side[bank]];
			(*tracker)++;
		case 1: // @ Calcula a experiência e define os Pokémon que receberão
        {
            u8 via_expshare = 0;
            u8 via_sentin = 0;
            ...
            for (u8 i = 0; i < 6; i++)
            {
                ...
                if (is_poke_usable(poke))
                {
                    ...
                    sent_in &= ~(BIT_GET(i)); // @ Sem experiência para Pokémon não utilizável
            }
    ...
    case 2: // @ Define valores de experiência e exibe mensagens
        {
            exp_for_poke = 0;
            ...
            if (is_poke_usable(poke) && (GETS_VIA_EXPSHARE(held_item) || *sentin_pokes & 1))
            {
                ...
                if (get_attributes(poke, ATTR_LEVEL, 0) < current_level_cap && !GET_CUSTOMFLAG(DISABLED_EXP_FLAG)) // @ Aplica experiência
                {
                    if (*sentin_pokes & 1)
                    ...
                    //@ Caso o Pokémon atinja o Level Cap, obtém o nível atual do Pokémon
					u8 current_level = get_attributes(poke, ATTR_LEVEL, 0);
					// Obtém a espécie do Pokémon
					u16 species = get_attributes(poke, ATTR_SPECIES, 0);
					// Verifica se o nível resultante ultrapassaria o Level Cap
					if (current_level + exp_for_poke / (*basestat_table)[species].exp_yield >= current_level_cap)
					{
						exp_for_poke = 0; // Bloqueia experiência se atingir ou exceder o Level Cap
					}
			...
			{
					// Pokémon atingiu o level cap ou nível máximo
					battle_text_buff1[0] = 0xFD;
					battle_text_buff1[1] = 4;
					battle_text_buff1[2] = battle_stuff_ptr->expgetter_bank;
					battle_text_buff1[3] = *exp_getter_id;
					...
					if (current_level_cap)
				{
					prep_string(0x25A, battle_stuff_ptr->expgetter_bank);
				}
				*tracker = 5; // apenas EVs foram atualizados
			}
		}
		else
		{
		...
		(*sentin_pokes) >>= 1; // limpar bits para que não tente dar experiência a esse Pokémon novamente
	}
	break;
		case 3: // @ Dá experiência e move a barra de experiência
...
const command bs_commands[] = {
		...
		(void*) 0x8056EDD,    //	 0xF7
		(void*) 0x8056EF9,    //	 0xF8
		atkF9_pollen_puff,    //	 0xF9
		atkFA_blowifnotdamp,    //	 0xFA
		atkFB_chloroblastEffect,    //	 0xFB
		atkFC_set_snow,    //	 0xFC
};
