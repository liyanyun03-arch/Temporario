...
u8 item_battle_effects(u8 switchid, u8 bank, u8 move_turn)
{
    ...
    case 2: //having effect on target items
		...
		u8 move_split = move_table[current_move].split & photon_geyser_special(current_move);
        switch (item_effect)
        {
		...
		case ITEM_EFFECT_JABOCABERRY:
			...
			{
				damage_loc = get_1_8_of_max_hp(bank_attacker); // Removida a lógica de Ripen
				effect = true;
				battle_scripting.active_bank = bank;
	...
	case 7: //Eject button
		...
		case ITEM_EFFECT_EJECTPACK:
			effect = item_force_switching(bank, BS_EJECTPACK_SWITCH,1);
			break;
		}
		break;
	}
