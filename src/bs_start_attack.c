...
void bs_start_attack(void) 
{
	...
	else if(!(bits_table[bank_attacker]&battle_stuff_ptr->absent_bank_flags_prev_turn)) 
	{
		...
		u8 change = 0; // Flag única para todas as transformações
		...
		// Marshadow - Só transforma se ainda não estiver na forma Zenit
		else if (*species == POKE_MARSHADOW && current_move == MOVE_SPECTRAL_THIEF && *species != POKE_MARSHADOW_ZENIT) 
		{
			*species = POKE_MARSHADOW_ZENIT;
			change = 1;
		}
		// Solgaleo - Só transforma se ainda não estiver na forma Radiant Sun
		else if (*species == POKE_SOLGALEON && current_move == MOVE_SUNSTEEL_STRIKE && *species != POKE_SOLGALEON_RADIANT_SUN) 
		{
			*species = POKE_SOLGALEON_RADIANT_SUN;
			change = 1;
		}
		// Lunala - Só transforma se ainda não estiver na forma Full Moon
		else if (*species == POKE_LUNALA && current_move == MOVE_MOONGEIST_BEAM && *species != POKE_LUNALA_FULL_MOON) 
		{
			*species = POKE_LUNALA_FULL_MOON;
			change = 1;
		}
		// Xerneas - Só transforma se ainda não estiver na forma Active
		else if (*species == POKE_XERNEAS && current_move == MOVE_GEOMANCY && *species != POKE_XERNEAS_ACTIVE) 
		{
			*species = POKE_XERNEAS_ACTIVE;
			change = 1;
		}

		// Executa animação APENAS se houve mudança efetiva
		if (change && is_bank_present(bank_attacker)) 
		{
