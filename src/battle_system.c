...
u16 get_airborne_state(u8 bank, u8 mode, u8 check_levitate)
{
    ...
    if (get_item_effect(bank, true) == ITEM_EFFECT_IRONBALL
        ...
        || status3[bank].rooted)
        return 1;
    if (check_levitate && battle_participants[bank].ability_id == ABILITY_LEVITATE && has_ability_effect(bank,0))
        return 4;
...
u16 type_effectiveness_calc(u16 move, u8 move_type, u8 atk_bank, u8 def_bank, u8 effects_handling_and_recording)
{
    ...
    {
        chained_effect = apply_type_effectiveness(move, chained_effect, move_table[move].arg2, def_bank, atk_bank, airstatus);
    }
...
bool is_boss_trainer(u8 bank)
{
    if (get_bank_side(bank) == 1)
        return 1;
    return 0;
}

bool is_elite_trainer(u8 bank) 
{
    enum trainer_class class = get_trainer_opponent_A_class();
    return (get_bank_side(bank) == 1 && 
	(
        class == CLASS_ELITE_FOUR || 
        class == CLASS_LEADER || 
        class == CLASS_PKMN_TRAINER_RIVAL || 
        class == CLASS_CHAMPION ||
        class == CLASS_AQUA_ADMIN ||
        class == CLASS_MAGMA_ADMIN ||
        class == CLASS_AQUA_LEADER ||
        class == CLASS_MAGMA_LEADER ||
        class == CLASS_DOME_ACE ||
        class == CLASS_PALACE_MAVEN ||
        class == CLASS_ARENA_TYCOON ||
        class == CLASS_FACTORY_HEAD ||
        class == CLASS_PIKE_QUEEN ||
        class == CLASS_PYRAMID_KING
    ));
}
...
bool has_hp_immunity(u8 bank)
{
    ...
    u16 max_hp = battle_participants[bank].max_hp;
    if (is_elite_trainer(bank))
        return (current_hp * 100 / max_hp > 50);
    if (is_boss_trainer(bank))
        return (current_hp * 100 / max_hp > 80);
    return false;
}
...
{   //0 == can poison
    ...
    if (has_hp_immunity(def_bank))
        return 4; // Mantendo coerente com imunidade de habilidade
...
u8 calculate_effect_chance(u8 bank, u16 move)
{
    ...
    if (has_hp_immunity(bank)) 
    {
        effect = (effect * 11) / 10; // Aumenta a chance de efeitos secundários em 10%
    }
        ...
        else
        {
            ...
            switch (*tracker)
            {
                ...
                case 21: //magnet rise
                    ...
                    break;
                case 22: //clear custap_eff,acc_up_nextturn,always hits and always crit flag
                ...
                case 30: //forms changes based on HP
                    {
						...
						void* script = NULL;
                            ...
                            case POKE_DARMANITAN:
                                if(battle_participants[activeBank].current_hp && (battle_participants[activeBank].current_hp < 
								(battle_participants[activeBank].max_hp / 2)) && check_ability(activeBank,ABILITY_ZEN_MODE))
                                {
                                    effect = 1;
                                ...
                                break;
                            case POKE_ZEN_MODE:
                                if((battle_participants[activeBank].current_hp >= (battle_participants[activeBank].max_hp / 2)) 
									|| !check_ability(activeBank,ABILITY_ZEN_MODE))
                                {
                                    effect = 1;
                                ...
                                break;
                            case POKE_DARMANITAN_GALAR:
                                if(battle_participants[activeBank].current_hp && (battle_participants[activeBank].current_hp < 
								(battle_participants[activeBank].max_hp / 2)) && check_ability(activeBank,ABILITY_ZEN_MODE))
                                {
                                    effect = 1;
                                    new_species = POKE_ZEN_MODE_GALAR;
                                    string_chooser = 0x21C;
                                    //battle_scripting.active_bank = activeBank;
                                    script = (BS_FORMCHANGE_WITH_TYPE_CHANGE);
                                }
                                break;
                            case POKE_ZEN_MODE_GALAR:
                                if((battle_participants[activeBank].current_hp >= (battle_participants[activeBank].max_hp / 2)) 
									|| !check_ability(activeBank,ABILITY_ZEN_MODE))
                                {
                                    effect = 1;
                                    new_species = POKE_DARMANITAN_GALAR;
                                    //battle_scripting.active_bank = activeBank;
                                    string_chooser = 0x21D;
                                    script = (BS_FORMCHANGE_WITH_TYPE_CHANGE);
                                }
                            ...
                            case POKE_WISHIWASHI:
                                if((battle_participants[activeBank].current_hp >= (battle_participants[activeBank].max_hp / 4)) && 
								check_ability(activeBank,ABILITY_SCHOOLING) && battle_participants[activeBank].level >=SCHOOLING_LEVEL)
                                {
                                    effect = 1;
                            ...
                            case POKE_WISHIWASHI_SCHOOL:
                                if(battle_participants[activeBank].current_hp && (battle_participants[activeBank].current_hp < 
								(battle_participants[activeBank].max_hp / 4)))
                                {
                                    effect = 1;
							...
							case 0x3F0:*/
                                if((battle_participants[activeBank].current_hp > (battle_participants[activeBank].max_hp / 2)) && 
								check_ability(activeBank,ABILITY_SHIELDS_DOWN))
                                {
                                    effect = 1;
                            ...
                            case POKE_MINIOR_METEOR:
                                if(battle_participants[activeBank].current_hp && (battle_participants[activeBank].current_hp <= 
								(battle_participants[activeBank].max_hp / 2)))
                                {
                                    effect = 1;									
                                ...
                                break;
							case POKE_MAGEARNA:
								// @ Verifica se Magearna tem menos ou igual a 50% de HP e se possui a habilidade Soul-Heart
								if (battle_participants[activeBank].current_hp &&
									(battle_participants[activeBank].current_hp <= (battle_participants[activeBank].max_hp / 2)) &&
									check_ability(activeBank, ABILITY_SOUL_HEART)) 
									{

									effect = 1; // @ Marca que haverá uma mudança de forma

									// @ Define o lado (user/AI) e ajusta os bits para Magearna Origin
									if (get_bank_side(activeBank)) 
									{
										if (species == POKE_MAGEARNA)
											new_battlestruct->party_bit.is_base_mag_ai |= bits_table[battle_team_id_by_side[activeBank]];
										else
											new_battlestruct->party_bit.is_base_mag_ai &= ~bits_table[battle_team_id_by_side[activeBank]];
									} 
									else 
									{
										if (species == POKE_MAGEARNA)
											new_battlestruct->party_bit.is_base_mag_user |= bits_table[battle_team_id_by_side[activeBank]];
										else
											new_battlestruct->party_bit.is_base_mag_user &= ~bits_table[battle_team_id_by_side[activeBank]];
									}

									new_species = POKE_MAGEARNA_ORIGINAL; // @ Define a nova forma (Magearna Origin)

									// @ Configura os textos e o script de mudança de forma
									string_chooser = 0x270; // @ ID para a string de mudança de forma
									script = (BS_FORMCHANGE_WITH_TYPE_CHANGE); // @ Script associado à mudança de forma
								}
								break;
                            //case POKE_ZYGARDE_10:
							case POKE_MARSHADOW_ZENIT:
								effect = 1;
								new_species = POKE_MARSHADOW;
								//battle_scripting.active_bank = activeBank;
                            ...
                            case POKE_ZYGARDE_50:
                                if(battle_participants[activeBank].current_hp && ((battle_participants[activeBank].current_hp <= 
								(battle_participants[activeBank].max_hp / 2)) && check_ability(activeBank,ABILITY_POWER_CONSTRUCT)))
                                {
                                    effect = 1;
...
{
    #define TURN_LAST_CASE 31
    bool effect = 0;
    for (bank_attacker = 0; bank_attacker < gBattlersCount; bank_attacker++)
    {
        if (!(absent_bank_flags & BIT_GET(bank_attacker)))
            break;
    }
    for (bank_target = 0; bank_target < gBattlersCount; bank_target++)
    {
    ...
    {   
		...
		case 11: // Reflect
			...
			{
				u8 ability = battle_participants[bank_attacker].ability_id; // Adiciona a obtenção da habilidade do atacante
				// Se o Pokémon atacante tem a habilidade Screen Cleaner, desativa imediatamente o Reflect
				if (ability == ABILITY_SCREEN_CLEANER && has_ability_effect(bank_attacker, ABILITY_SCREEN_CLEANER))
				{
					// Força a desativação do Reflect assim que o Pokémon entra no campo
					if (side_affecting_halfword[*sidebank].reflect_on)
					{
						side_affecting_halfword[*sidebank].reflect_on = 0; // Remove Reflect
						bank_attacker = side_timers[*sidebank].reflect_bank;
						effect = 1;
						call_bc_move_exec((void*)0x82DACFA); // Chama uma ação de comunicação
						move_to_buff1(MOVE_REFLECT); // Se necessário, pode ser alterado para outro efeito
					}
				}
				else
				{
					// Caso o Screen Cleaner não esteja ativo, verifica o Reflect normalmente
					if (side_affecting_halfword[*sidebank].reflect_on)
					{
		...
		case 12: // Light Screen
			...
			{
				u8 ability = battle_participants[bank_attacker].ability_id; // Adiciona a obtenção da habilidade do atacante
				// Se o Pokémon atacante tem a habilidade Screen Cleaner, desativa imediatamente o Light Screen
				if (ability == ABILITY_SCREEN_CLEANER && has_ability_effect(bank_attacker, ABILITY_SCREEN_CLEANER))
				{
					// Força a desativação do Light Screen assim que o Pokémon entra no campo
					if (side_affecting_halfword[*sidebank].light_screen_on)
					{
						side_affecting_halfword[*sidebank].light_screen_on = 0; // Remove Light Screen
						bank_attacker = side_timers[*sidebank].lightscreen_bank;
						effect = 1;
						call_bc_move_exec((void*)0x82DACFA); // Chama uma ação de comunicação
						battle_communication_struct.multistring_chooser = *sidebank;
						move_to_buff1(MOVE_LIGHT_SCREEN); // Se necessário, pode ser alterado para outro efeito
					}
				}
				else
				{
					// Caso o Screen Cleaner não esteja ativo, verifica o Light Screen normalmente
					if (side_affecting_halfword[*sidebank].light_screen_on)
					{
		...
		case 13: // Aurora Veil
			...
			{
				u8 ability = battle_participants[bank_attacker].ability_id; // Adiciona a obtenção da habilidade do atacante
				// Se o Pokémon atacante tem a habilidade Screen Cleaner, desativa imediatamente o Aurora Veil
				if (ability == ABILITY_SCREEN_CLEANER && has_ability_effect(bank_attacker, ABILITY_SCREEN_CLEANER))
				{
					// Força a desativação do Aurora Veil assim que o Pokémon entra no campo
					if (new_battlestruct->side_affecting[*sidebank].aurora_veil)
					{
						new_battlestruct->side_affecting[*sidebank].aurora_veil = 0; // Remove Aurora Veil
						bank_attacker = *sidebank;
						effect = 1;
						call_bc_move_exec((void*)0x82DACFA); // Chama uma ação de comunicação
						battle_communication_struct.multistring_chooser = *sidebank;
						move_to_buff1(MOVE_AURORA_VEIL); // Se necessário, pode ser alterado para outro efeito
					}
				}
				else
				{
					// Caso o Screen Cleaner não esteja ativo, verifica o Aurora Veil normalmente
					if (new_battlestruct->side_affecting[*sidebank].aurora_veil)
					{
        ...
        case 20: //rainbow
            ...
            break;
        case 21: //wish
            while (*sidebank < gBattlersCount && effect == 0)
            {
...
bool is_screen_cleaner_active() 
{
    for (u8 i = 0; i < gBattlersCount; i++) 
	{
        if (battle_participants[i].ability_id == ABILITY_SCREEN_CLEANER && has_ability_effect(i, 0)) 
		{
            return true;
        }
    }
...
u8 check_move_limitations(u8 bank, u8 not_usable_moves, struct move_limitation limitations) 
{
    ...
    bool screen_cleaner_active = is_screen_cleaner_active(); // Verifica se Screen Cleaner está ativo em qualquer Pokémon no campo
    ...
    for (u8 i = 0; i < 4; i++) 
	{
        ...
        if (move_to_check == 0 && limitations.limitation_no_move)
            ...
            not_usable_moves |= BIT_GET(i);
        else if (new_battlestruct->bank_affecting[bank].throatchop_timer && find_move_in_table(move_to_check, sound_moves))
            not_usable_moves |= BIT_GET(i);
        else if (screen_cleaner_active && (move_to_check == MOVE_REFLECT || move_to_check == MOVE_LIGHT_SCREEN || move_to_check == MOVE_AURORA_VEIL))
            not_usable_moves |= BIT_GET(i); // Bloqueia o movimento se Screen Cleaner estiver ativo
...
bool message_cant_choose_move(void)
{
    ...
    // Verifica se algum Pokémon no campo tem Screen Cleaner
    for (u8 i = 0; i < 4; i++) // Itera sobre os 4 possíveis bancos
    {
        if ((battle_participants[i].ability_id == ABILITY_SCREEN_CLEANER) && has_ability_effect(i, 0))
        {
            // Checa se o movimento é Reflect, Light Screen ou Aurora Veil
            if (checking_move == MOVE_REFLECT || 
                checking_move == MOVE_LIGHT_SCREEN || 
                checking_move == MOVE_AURORA_VEIL)
            {
                cant = 1;
                *loc_to_store_bs = BS_SCREEN_CLEANER_BLOCKED; // Endereço do Battle Script para Screen Cleaner
                break; // Já encontramos Screen Cleaner, não precisamos continuar verificando
            }
        }
    }
    ...
    if (!cant)
    {
		...
		else if (new_battlestruct->bank_affecting[bank].throatchop_timer && find_move_in_table(checking_move, sound_moves))
		{
			cant = 1;
			*loc_to_store_bs = BS_CANTSELECT_THROATCHOP;
		}
        ...
        for (u8 i = 0; i < 6; i++)
        {
            if (can_poke_be_switched_into(i, self))
            {
                for (u8 j = 0; j < 4; j++)
                {
                    ...
                    {
                        battle_stuff_ptr->switchout_index[self] = i;  
