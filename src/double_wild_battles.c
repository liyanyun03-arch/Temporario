...
#define DOUBLE_BATTLE_PERCENT 60
...
void pokeball_chosen(u8 taskID)
{
    ...
    if (WILD_DOUBLE_BATTLE &&
        ...
        {
            static const u8 text_impossible_to_aim[] = {0x06, Space, i_, m_, p_, o_, s_, s_, 0x6F, v_, e_, l_, Space, m_, i_, r_, a_, r_, Space, s_, e_, m_, Space, e_, s_, t_, a_, r_, Space, f_, o_, c_, a_, d_, o_, Exclam, wait_for_pressed_key, 0xFF};
...
{
	return (species > 846 && species < 853);
}

#define BALL_BEAST 0x1A
	...
	else
{
    ...
    case BALL_MOON:
        {
            ...
            {
                if (evo[i].method == 7 && evo[i].paramter == ITEM_MOON_STONE)
                {
                    multiplier = 40;
                    break;
                }
	...
	case BALL_DREAM:
	{
		if (catching->status.flags.sleep)
			multiplier = 40;
		else
			multiplier = 10;
	}
