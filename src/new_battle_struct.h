...
struct bank_affecting{
    ...
    u8 always_crit : 2;
    u8 itemheal : 1;
	u8 throatchop_timer : 2; 	
	u8 rage_powder : 1;
	u8 beak_blast_charge : 2;
	u8 shell_trap_charge : 2;
	u8 lastmove_fail : 1;
	u8 head_blown : 1;
	u8 head_blow_hpupsdate : 1; //used in head blow and Steel Beam
	u8 stockpile_counter : 2;
	u8 move_worked_thisturn : 1;
	u8 be_intimidated : 1;
	u8 acc_up_nextturn : 2;
	u8 custap_eff_nextturn : 2;
	...
	u8 chloroblast_recoil: 1;
	u8 chloroblast_activated: 1;
};
...
struct various{
	...
	u8 abilityswitch_atkbank : 2;
	u8 abilityswitch_defbank : 2;
	u8 activating_bankorder_while_gas_leaving : 3;//activing bank order while neutralizing_gas leaving
	u8 intimidating : 1;
	u8 throatspray : 1;
	u8 mirror_armor_affecting : 1;
	u8 hailing_this_turn : 1;
	u8 neutralizing_gas_in_force : 1;
